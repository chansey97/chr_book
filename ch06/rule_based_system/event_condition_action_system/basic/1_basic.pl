...
upd @ update(t(P),t(C1)) \ t(C) <=> match(P,C) | t(C1).
...
%% 3. update 如果 update 相同的数据，会死循环（postgres 和 sqlite 不会死循环）
%% 这是因为在upd rule里，产生的 t(C) 总是可以和当前的 active constraint update(P,C1) 里的 C1 unify
%% see 2_basic_fix_loop.pl to fix
