...
%% However, the four rules are still not sufficient, more propagations are possible.
%% For example, consider [R1,R2,R3] lex [T1,T2,T3], R2=T2, R3>T3.
%% The only way to satisfy this constraint is by imposing R1<T1, since the
%% remaining elements cannot be ordered in the right way if R1>=T1.

%% ?- [R1,R2,R3] lex [T1,T2,T3], bi(R2=T2), bi(R3>T3).
%@ R2 = $VAR(T2),
%@ [$VAR(R1),$VAR(T2),$VAR(R3)]lex[$VAR(T1),$VAR(T2),$VAR(T3)],
%@ bi($VAR(R3)=\= $VAR(T3)),
%@ bi($VAR(R3)>= $VAR(T3)),
%@ bi($VAR(R3)> $VAR(T3)),
%@ bi($VAR(R1)=< $VAR(T1)).
