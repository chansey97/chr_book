...
%% There are still situations that are not covered by the current set of rules.
%% Just replace R2=T2 in the above example by R2>=T2. The same propagation
%% should take place as before, but the two rules that we have added cannot be
%% applied, their guards are too strict.

%% ?- [R1,R2,R3] lex [T1,T2,T3], bi(R2>=T2), bi(R3>T3).
%@ [$VAR(R1),$VAR(R2),$VAR(R3)]lex[$VAR(T1),$VAR(T2),$VAR(T3)],
%@ bi($VAR(R3)=\= $VAR(T3)),
%@ bi($VAR(R3)>= $VAR(T3)),
%@ bi($VAR(R3)> $VAR(T3)),
%@ bi($VAR(R2)>= $VAR(T2)),
