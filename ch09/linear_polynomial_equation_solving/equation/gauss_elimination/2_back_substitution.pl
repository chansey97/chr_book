...
%% Add back substitution (back substitution once).
...
bound @     backsubst \ P eq C <=> select(A*X, P, P0), number(X) | C0 is C-A*X, P0 eq C0.

%% A*X = C  =>  bind variable
bind @      backsubst \ [A*X] eq C <=> var(X) | X is C/A.
...
%% ?- globalize([X,Y]), [1*X,3*Y] eq -5, [3*X,2*Y] eq -8, backsubst.
%@ X = -2,
%@ Y = -1,
%@ backsubst ;
%@ false.
...
%@ [11.0* $VAR(Z)]eq 18.0,
%@ [0.5* $VAR(Y),1.0* $VAR(Z)]eq 1.0,
%@ [2* $VAR(X),3* $VAR(Y),4* $VAR(Z)]eq 6 ;
%@ false.

%% ?- globalize([X,Y,Z]), [2*X,3*Y,4*Z] eq 6, [1*X,2*Y,3*Z] eq 4, [3*X,-4*Y] eq 10, backsubst.
...
%% ?- globalize([A,B,C]), [1*A,1*B,-1*C] eq 2, [2*A,3*B,-1*C] eq 0, [3*A,4*B,-2*C] eq 1, backsubst.
%@ false.
...
%% ?- globalize([X,Y,Z]), [1*X,1*Y,1*Z] eq 3, [2*X,4*Y,1*Z] eq 8, [6*X,10*Y,4*Z] eq 22, backsubst.
%@ [2* $VAR(Y),-1* $VAR(Z)]eq 2,
%@ [1* $VAR(X),1* $VAR(Y),1* $VAR(Z)]eq 3,
%@ backsubst ;
%@ false.

%% Fix a back substitution problem in old code

%% ?- globalize([X,Y,Z]), [2*X,4*Y,2*Z] eq -2, [2*X,4*Y,4*Z] eq -4, [2*X,3*Z] eq -1.
%@ [-4* $VAR(Y),1* $VAR(Z)]eq 1,
%@ [2* $VAR(Z)]eq-2,
%@ [2* $VAR(X),4* $VAR(Y),2* $VAR(Z)]eq-2 ;
%@ false.

%% ?- globalize([X,Y,Z]), [2*X,4*Y,2*Z] eq -2, [2*X,4*Y,4*Z] eq -4, [2*X,3*Z] eq -1, backsubst.
%@ X = 1.0,
%@ Y = -0.5,
%@ Z = -1,
%@ backsubst ;
